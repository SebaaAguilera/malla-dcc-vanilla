const createElement = (tag, attributes, ...children) => {
  ...
  Object.entries(attributes).forEach(([key, attr]) => element.setAttribute(key, attr))
...
const createSubject = (subject) => {
  ...
  let subjectDiv = createElement('div', { id: subject.code, class: 'subject' }, paragraph)
  ...
  subjectDiv.onmouseover = () => {
    ...
    subject.requirements.forEach(requirement => {
      if (requirement.endsWith('S')) {
        ...
        requirement.split('/').forEach(requirement => changeStyleById(requirement, 'or'))
      } else {
  ...
  subjectDiv.onmouseout = () => {
    ...
    subject.requirements.forEach(requirement => {
      if (requirement.endsWith('S')) {
        ...
        requirement.split('/').forEach(requirement => changeStyleById(requirement))
      } else {
